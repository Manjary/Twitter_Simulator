defmodule DataCentre do
    ...
    def handle_cast({:getmentioned, mention,spid}, state) do
        IO.puts "I am in getMentioned"
        
        GenServer.cast({:global,:querysimulator},{:receivedmentioned,state[:mentions][mention]})
        
        {:noreply, state}
    end


     def handle_cast({:getsubscribertweetdc, name,spid}, state) do
        IO.inspect name
        IO.inspect(state[:tweets_by_user][name])
        #GenServer.cast({:global,:querysimulator},{:receivedsubscribertweet,state[:tweets_by_user][name]})
